...
for (let letterToRemove = 0; letterToRemove < 26; letterToRemove++) {
  const solutions = new Set();

  for (let i = 0; i < words.length; i++) {
    const word = words[i];
    const croppedWord =
      word.slice(0, letterToRemove) + word.slice(letterToRemove + 1, 26);
    if (solutions.has(croppedWord)) {
      result = croppedWord;
      break;
    }
    solutions.add(croppedWord);
  }
  if (result) {
    break;
  }
